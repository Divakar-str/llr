...
async function initializeReportsEnginePipeline() {
    ...
    document.getElementById("generateReportBtn").addEventListener("click", processReportTemplateMapping);
    document.getElementById("exportExcelBtn").addEventListener("click", exportActiveTableToExcelFile);
    document.getElementById("exportCsvBtn").addEventListener("click", exportActiveTableToCsvFile);
    document.getElementById("printReportBtn").addEventListener("click", triggerLandscapePrintProcess);
    ...
    try {
        ...
        // SUCCESS HANDLING: Flash feedback trace count to operators console
        traySpinner.classList.add("d-none");
        trayIcon.className = "bi bi-check-circle-fill text-success fs-5";
        statusTray.className = "alert bg-success-subtle d-flex align-items-center gap-2 py-2 px-3 shadow-sm scale-in";
        trayMessage.innerHTML = `<strong>Connected!</strong> Successfully loaded <span class="badge bg-success px-2 py-1">${masterRegistryData.length} records</span> straight from spreadsheet logs.`;

        setTimeout(() => { statusTray.style.setProperty("display", "none", "important"); }, 4000);

    } catch (e) {
        traySpinner.classList.add("d-none");
        trayIcon.className = "bi bi-exclamation-triangle-fill text-danger fs-5";
        statusTray.className = "alert bg-danger-subtle d-flex align-items-center gap-2 py-2 px-3 shadow-sm scale-in";
        trayMessage.innerHTML = `<strong>Fetch Error!</strong> Could not read database records. Check your internet connection or API endpoint setup.`;
    }
}
...
function evaluateRowLogicalStatus(row, currentTimestamp) {
    ...
    const isDlIssued = row.dl_issued && row.dl_issued.toUpperCase() === "YES";
    ...
    if (iss) {
        state.daysSinceIssue = Math.ceil((currentTimestamp - iss) / (1000 * 60 * 60 * 24));
...
/**
 * FIXED RESET TOGGLE MATCHES: Triggers absolute tracking bounds.
 */
function matchMinimalFilters(row) {
    ...
    if (query !== "") {
        ...
        if (!nameMatch && !llrMatch && !dlMatch && !mobMatch) return false;
    }

    // Resolves ALL toggling reset states bugs smoothly
    if (classFilter !== "ALL") {
    ...
    const iss = parseCustomDate(row.issue_date);
    if (fromDateStr && iss) {
        const start = new Date(fromDateStr); start.setHours(0, 0, 0, 0);
        if (iss < start) return false;
    }
    if (toDateStr && iss) {
        const end = new Date(toDateStr); end.setHours(23, 59, 59, 999);
...
function processReportTemplateMapping() {
    ...
    masterRegistryData.forEach(row => {
        ...
        // FIXED CRASH ISSUE: Date extraction placed safely at loop header
        ...
        if (row.dl_issued && row.dl_issued.toUpperCase() === "YES") ribbonTallies.dlIssued++;

        let entry = {
            "S.No.": displaySerialNo, "LLR Number": row.llr_number, "Applicant Name": row.name,
            "Vehicle Class": row.vehicle_class, "Mobile Number": row.mobile_number,
            "Issue Date": row.issue_date, "Expiry Date": row.expiry_date
        };

        switch (reportEngineKey) {
            case "COMPLETE_REGISTER":
                activeHeaderTemplate = [...commonHeaders, "Date of Birth", "Blood Group", "Address Details"];
                Object.assign(entry, { "Date of Birth": row.date_of_birth, "Blood Group": row.blood_group, "Address Details": row.present_address });
                break;
            case "ACTIVE_LLR": if (!state.isActive) return; break;
            case "EXPIRED_LLR":
            ...
            case "ELIGIBLE_FOR_DL":
                ...
                break;
            case "PENDING_DL":
                if (row.dl_issued && row.dl_issued.toUpperCase() === "YES") return;
            ...
            case "DL_ISSUED":
                ...
                entry["Permanent DL Number"] = row.dl_number; entry["Approval Date"] = row.approved_date;
                break;
            case "LLR_TO_DL_CONVERSION":
                activeHeaderTemplate = [...commonHeaders, "Permanent DL Status"];
                entry["Permanent DL Status"] = row.dl_issued === "Yes" ? `License Done (${row.dl_number})` : "Learner Stage Only";
                break;
            ...
            case "EXPIRING_30_DAYS":
                ...
                break;
            case "TODAYS_ISSUED":
                const sysTodayStr = `${String(today.getDate()).padStart(2, '0')}-${String(today.getMonth() + 1).padStart(2, '0')}-${today.getFullYear()}`;
                if (row.issue_date !== sysTodayStr) return;
                break;
            case "MONTHLY_ISSUED":
            ...
            case "DUPLICATE_MOBILE":
                const mCount = masterRegistryData.filter(x => x.mobile_number === row.mobile_number).length;
                if (mCount <= 1) return;
...
function renderCompiledReportTableToView(headers, dataRows) {
    ...
    document.getElementById("masterPrintCheckbox").addEventListener("change", (event) => {
